    ...
    <>
      {agentsList.map((e) => (
        <Link to={`/agent/${e.id}`} className="grid xs:flex sm:grid sm:grid-cols-2 md:grid-cols-1 xl:grid-cols-2 bg-white rounded-2xl sm:rounded-3xl md:rounded-[32px] 2xl:rounded-[58px] overflow-hidden card cursor-pointer  p-4 xs:p-0 md:p-4 xl:p-0">
          <div className="h-full w-full xs:max-w-[180px] sm:max-w-full overflow-hidden rounded-2xl xs:rounded-none md:rounded-3xl xl:rounded-none">
            <img src={e.img} alt="" className="h-full w-full object-cover" />
          </div>
          <div className=" pt-5 pb-2 xs:py-6 2xl:py-8 px-3 md:px-2 xl:px-4 2xl:pr-9">
